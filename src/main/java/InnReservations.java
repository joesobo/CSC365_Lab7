...
public class InnReservations {
  ...
  private static void makeReservation(Scanner in) {
    ...
    try (
      ...
      try (
        PreparedStatement pStatement = conn.prepareStatement(sqlInsertString)
      ) {
        ...
        if (pStatement.executeUpdate() > 0) {
          String sqlConfirmationString =
            "SELECT CODE, FirstName, LastName, RoomCode, RoomName, bedType, CheckIn, " +
            "CheckOut, Adults, kids, Rate FROM lab7_reservations JOIN lab7_rooms on Room = RoomCode " +
            "WHERE CheckIn = ? and CheckOut = ? and Room = ?";

          try (
            PreparedStatement confirmationPStatement = conn.prepareStatement(
              sqlConfirmationString
            )
          ) {
            confirmationPStatement.setDate(1, checkIn);
            confirmationPStatement.setDate(2, checkOut);
            confirmationPStatement.setString(3, roomCode);
            ResultSet resultSet = confirmationPStatement.executeQuery();
            ...
            if (resultSet.next()) {
              ...
              LocalDate confirmCheckIn = resultSet
                .getDate("CheckIn")
                .toLocalDate();
              LocalDate confirmCheckOut = resultSet
                .getDate("CheckOut")
                .toLocalDate();
              ...
              long weekendDays = Stream
                .iterate(confirmCheckIn, date -> date.plusDays(1))
                .limit(totalDays)
                .filter(isWeekend)
                .count();
      ...
      System.err.println(
        "Could not create reservation: " + e.getMessage().split(";")[0] + "\n"
      );
    }
  }
  ...
  private static void changeReservation(Scanner in) {
    ...
    System.out.println(
      "\n***If you do not want to change a field please enter " +
      noChange +
      "***\n"
    );
    ...
    if (!firstName.equalsIgnoreCase(noChange)) {
      stringJoiner.add("FirstName = ?");
      values.add(firstName);
    }
    if (!lastName.equalsIgnoreCase(noChange)) {
      stringJoiner.add("LastName = ?");
      values.add(lastName);
    }
    if (!checkInString.equalsIgnoreCase(noChange)) {
      stringJoiner.add("CheckIn = ?");
      values.add(Date.valueOf(checkInString));
    }
    if (!checkOutString.equalsIgnoreCase(noChange)) {
      stringJoiner.add("CheckOut = ?");
      values.add(Date.valueOf(checkOutString));
    }
    if (!childrenString.equalsIgnoreCase(noChange)) {
      stringJoiner.add("Kids = ?");
      values.add(Integer.valueOf(childrenString));
    }
    if (!adultsString.equalsIgnoreCase(noChange)) {
  ...
  private static void cancelReservation(Scanner in) {
    ...
    try {
      ...
      try (
        ...
        String sqlStatement =
          "SELECT CODE FROM lab7_reservations WHERE CODE = ?";
        ...
        try (
          ...
          if (!rs.next()) {
            System.out.println(
              "Reservation: " + code + ", Could not be located\n"
            );
            return;
          }
        ...
        System.out.println(
          "Reservation: " + code + ", Could not be canceled\n"
        );
      }
    } catch (InputMismatchException e) {
      ...
      try (
        ...
        while (rs.next()) {
          ...
          System.out.format(
            ...
            tempCode,
            roomName,
            formatYear(months, total)
          );
        }
        System.out.format(
          "%-31s %s %n",
          "FINAL TOTAL:",
          formatYear(monthFinalRevenue, finalRevenue)
        );
      }
    } catch (SQLException e) {
  ...
  private static String formatYear(float[] totalArr, float total) {
    String output = "";
    ...
    return output + String.format("| %-9s", monthTotal);
  }
}
